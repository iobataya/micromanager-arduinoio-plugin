...
/**
 * This class was moved to org.micromanager.overlayarduino pakcage.
 * @author jpk10
 *
 */
public class AcqByTtlMigForm extends MMDialog implements ArduinoInputListener {
	...
	public AcqByTtlMigForm(MMStudio gui)  {
		...
		gui_.getAcquisitionEngine().getImageProcessors().add(subtract_);

		arduino_ = new OverlayArduinoProcessor();
		arduino_.setApp(gui_);
		arduino_.makeConfigurationGUI();
		gui_.getAcquisitionEngine().getImageProcessors().add(arduino_);
		arduino_.process(); // start poller thread.

		// tell processors instance
		setSubtractBGProcessor(subtract_);
		setOverlayArduinoProcessor(arduino_); // this will set listener, too
